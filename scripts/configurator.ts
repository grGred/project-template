...
async function main() {
    let statuses = await Promise.allSettled(
        blockchains.map(async blockchain => {
            ...
            console.log(
                `${clc.blue(blockchain.name)}: (skipped ${skippedAmount}) adding routers: `,
                routersToAdd
            );

            if (routersToAdd.length > 0) {
                const overrides = blockchain.gasPrice
                    ? { gasPrice: ethers.utils.parseUnits(blockchain.gasPrice, 'gwei') }
                    : {};
    ...
    for (let i in statuses) {
        ...
        if (status.status == 'rejected') {
            console.log(
                `${clc.red(blockchains[i].name)}`,
                clc.red('ERROR :'),
                `${(<PromiseRejectedResult>statuses[i]).reason}`
            );
        } else {
